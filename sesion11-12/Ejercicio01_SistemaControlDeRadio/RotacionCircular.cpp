...
namespace EIF201 {
    ...
    bool RotacionCircular::insertarDespuesDe(const string& referencia,
        ...
        do {
            if (actual->nombre == referencia) {
                ...
                if (actual == cola) {
                    cola = nuevo; // si era la cola, el nuevo es la nueva cola
                }
    ...
    bool RotacionCircular::existeLocutor(
    ...
    string RotacionCircular::locutorMasActivo() const {
        ...
        NodoLocutor* mayor = cabeza;

        do {

            if (actual->turnosAsignados >
                mayor->turnosAsignados) {
